 ...
 */
package org.apache.nutch.parse.nasdaq;
 ...
 */
public class NasdaqParsingFilter extends RegexURLFilterBase implements ParseFilter {
  public static final Logger LOG = LoggerFactory
      .getLogger(NasdaqParsingFilter.class);
...
//-------------------------------------------------------------------------------------
public NasdaqParsingFilter() {
  super();
}

  public NasdaqParsingFilter(String filename) throws IOException,
          PatternSyntaxException {
    super(filename);
  }

  NasdaqParsingFilter(Reader reader) throws IOException, IllegalArgumentException {
    super(reader);
  }
  @Override
  ...
  protected Reader getRulesReader(Configuration conf) throws IOException {
    String fileRules = conf.get(NASDAQ_REGEX_FILE);
  ...
  public static final String NASDAQ_REGEX_FILE = "nasdaq.regex.file";
  //----------------------------------------------------------------------------------
...
//  }
  ...
  private Combine walk(DocumentFragment root){
    ...
    while(walker.hasNext()){
      ...
      if("table".equalsIgnoreCase(nodeName) && currentNode.getAttributes().getNamedItem("id")!=null && currentNode.getAttributes().getNamedItem("id").getNodeValue().equals("ECCompaniesTable")){
        NodeList nodeList=currentNode.getChildNodes();
        for(int i=0;i<nodeList.getLength();i++){
          Node childNode=nodeList.item(i);
          if("tbody".equalsIgnoreCase(childNode.getNodeName())){
            //System.out.println(childNode.getTextContent());
            NodeList rowList=childNode.getChildNodes();
            for(int j=0;j<rowList.getLength();j++){
              Node row=rowList.item(j);
              if(!row.getNodeName().equalsIgnoreCase("tr"))
                continue;
              NodeList tdList=row.getChildNodes();
              int second=0;
              for(int k=0;k<tdList.getLength();k++){
                Node td=tdList.item(k);
                if(td.getNodeName().equalsIgnoreCase("td")) {
                  if(second==0) {
                    second++;
                    continue;
                  }
                  if(second==1) {
                    String st=td.getTextContent().replace("\\s+"," ").trim();
                    sb.append(st.substring(0,st.indexOf("Market Cap")).trim()+"|");
                    sb.append(st.substring(st.indexOf(":")+2,st.length())+"|");
                  }else{
                    sb.append(td.getTextContent().replace("\\s+"," ").trim()+"|");
                  }
                  second++;
                }
              }
