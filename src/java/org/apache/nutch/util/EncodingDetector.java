...
public class EncodingDetector {
  ...
  private void autoDetectClues(ByteBuffer dataBuffer, CharSequence typeUtf8,
    ...
    LOG.info("encoding detector type is:"+type);
    LOG.info("encoding detector encoding is:"+encoding);
    if (minConfidence >= 0 && DETECTABLES.contains(type) && length > MIN_LENGTH) {
      LOG.info("encoding detector into detectables");
...
//    }
    LOG.info(baseUrl + ": Choosing encoding: " + bestClue);
