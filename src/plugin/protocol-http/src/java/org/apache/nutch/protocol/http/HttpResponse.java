...
public class HttpResponse implements Response {
  ...
  public HttpResponse(HttpBase http, URL url, WebPage page)
    ...
    try {
      ...
      Http.LOG.info("accept:"+this.http.getAccept());
      reqStr.append("\r\n");
      ...
      if ((userAgent == null) || (userAgent.length() == 0)) {
        ...
        Http.LOG.info("userAgent:"+userAgent);
        reqStr.append("\r\n");
      }
      ...
      headers.set("Protocal-Type","http");

      String transferEncoding = getHeader(Response.TRANSFER_ENCODING);
      if (transferEncoding != null
          && "chunked".equalsIgnoreCase(transferEncoding.trim())) {
        Http.LOG.info("encode chunked");
        readChunkedContent(in, line);
      } else {
        Http.LOG.info("encode plain");
        readPlainContent(in);
      }
